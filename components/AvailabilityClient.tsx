  ...
  return (
    <div className="min-h-screen bg-[#080808] text-white flex flex-col selection:bg-[#36B7F0] selection:text-black">
      ...
      <main className="flex-1 pt-24 pb-16">
        ...
        {/* Header / Intro Hero */}
        <section className="relative py-12 md:py-16 border-b border-[#292929] bg-[#0b0b0b]">
          <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(54,183,240,0.04),transparent_70%)] pointer-events-none" />
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative z-10">
            <div>
              ...
              <p className="text-zinc-400 text-sm sm:text-base max-w-xl leading-relaxed">
                Cek status unit XPLAY secara real-time sebelum datang. Dapatkan informasi ketersediaan konsol terhangat langsung dari billing lounge.
              </p>
            </div>
        ...
        <section className="py-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {loading && !data && (
            <div className="py-24 text-center">
              ...
              <p className="text-zinc-400 text-sm font-semibold">Menghubungkan ke server billing lounge...</p>
            </div>
          )}
              ...
              {filteredStations.map((station) => {
                ...
                return (
                  <div
                    ...
                    className={`rounded-2xl p-5 border flex flex-col justify-between transition-all group relative overflow-hidden ${
                      isVVIP
                  ...
                  >
                    {/* VIP/VVIP Badge */}
                    {(isVIP || isVVIP) && (
                      <div className="absolute top-0 right-0 px-3 py-1 bg-gradient-to-r text-black font-mono font-black text-[9px] uppercase tracking-widest rounded-bl-xl flex items-center gap-1 shadow-md" style={{
                        backgroundImage: isVVIP 
                          ? "linear-gradient(to right, #B45CE0, #FF3038)" 
                          : "linear-gradient(to right, #36B7F0, #75D84B)"
                      }}>
                        <Crown className="w-2.5 h-2.5" />
                        <span>{isVVIP ? "VVIP LOUNGE" : "VIP ROOM"}</span>
                      </div>
                    )}
                    ...
                    <div>
                      ...
                      <div className="flex items-start justify-between gap-4 mb-4">
                        <div>
                          ...
                          <h4 className="text-lg font-black text-white group-hover:text-[#36B7F0] transition-colors">
                            {station.stationName}
                          </h4>
                        </div>
                        <span className="px-2.5 py-1 rounded bg-[#171717] border border-[#292929] text-[10px] font-mono font-bold text-zinc-400 uppercase tracking-wider">
                          {station.consoleType}
                        </span>
                      </div>
                      ...
                      <div className="mb-6">
                        <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold border uppercase ${
                          isAvailable
                    ...
                    <div>
                      <div className="min-h-[48px] flex items-center justify-between border-t border-[#1f1f1f] pt-4 mb-4 text-xs font-medium text-zinc-400">
                        {isAvailable && (
                          <span className="text-[#75D84B] font-semibold">Siap dimainkan sekarang</span>
                        )}
                        {isPlaying && (
                          <div className="flex flex-col gap-0.5">
                            <span className="text-zinc-500">Estimasi Selesai:</span>
                            <span className="text-white font-extrabold font-mono text-sm flex items-center gap-1">
                              <Clock className="w-3.5 h-3.5 text-[#FF3038]" />
                              <span>± {station.estimatedFinishAt || "Open Billing"}</span>
                            </span>
                          </div>
                        )}
                        {isPaused && (
                          <span className="text-[#FFD84D]">Sesi dihentikan sementara</span>
                        )}
                        {station.status === "unavailable" && (
                          <span className="text-zinc-500">Unit tidak aktif / maintenance</span>
                        )}

                        {/* Queue Indicator */}
                        {station.queueCount > 0 && (
                          <span className="px-2 py-0.5 rounded bg-[#FFD84D]/10 border border-[#FFD84D]/20 text-[#FFD84D] text-[10px] font-mono font-bold">
                            Antrian: {station.queueCount}
                          </span>
                        )}
                      </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center max-w-3xl mx-auto mb-12">
              ...
              <p className="text-zinc-400 text-sm">
                Harap bedakan layanan mabar di lounge kami dengan produk rental bawa pulang ke rumah demi kenyamanan bersama.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <div className="bg-[#111111] border border-[#36B7F0]/30 rounded-3xl p-6 flex flex-col justify-between hover:border-[#36B7F0] transition-colors">
                <div>
                  ...
                  <h3 className="text-xl font-bold text-white mb-2">1. Main di XPLAY (Lounge)</h3>
                  <p className="text-xs sm:text-sm text-zinc-400 leading-relaxed mb-6">
                    Bermain langsung di gaming lounge premium kami. Dilengkapi setup layar lebar, ruangan VIP AC, sofa nyaman, makanan/minuman lengkap, dan atmosfir lounge yang seru.
                  </p>
                </div>
                <Link
                  href="/#pricing"
